...
class Action(BaseModel):
    ...
    def validate_action_payload(self) -> "Action":
        if self.action_type == ActionType.ASSIGN_CLASS.value and self.assign_class is None:
            raise ValueError("assign_class payload required for ASSIGN_CLASS action")
        if self.action_type == ActionType.RESCHEDULE_CLASS.value and self.reschedule_class is None:
            raise ValueError("reschedule_class payload required for RESCHEDULE_CLASS action")
        if self.action_type == ActionType.REMOVE_ASSIGNMENT.value and self.remove_assignment is None:
