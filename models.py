...
class Action(BaseModel):
    ...
    @validator("assign_class", always=True)
    def validate_assign(cls, v, values):
        if values.get("action_type") == ActionType.ASSIGN_CLASS.value and v is None:
            raise ValueError("assign_class payload required for ASSIGN_CLASS action")
        return v

    @validator("reschedule_class", always=True)
    def validate_reschedule(cls, v, values):
        if values.get("action_type") == ActionType.RESCHEDULE_CLASS.value and v is None:
            raise ValueError("reschedule_class payload required for RESCHEDULE_CLASS action")
        return v

    @validator("remove_assignment", always=True)
    def validate_remove(cls, v, values):
        if values.get("action_type") == ActionType.REMOVE_ASSIGNMENT.value and v is None:
